...
impl Session {
    ...
    async fn check_incoming_connection(
        ...
        // PATCHED FOR SERMONINDEX — wake-on-demand serving (not upstream
        // behaviour). A SermonIndex seed node keeps most of its ~25,500
        // torrents PAUSED so a small ARM board's memory stays flat (see the
        // discovered-peer cap patch in torrent_state/live/peers/mod.rs for the
        // history). Clients learn this node's address from the SermonIndex
        // node directory and dial it directly — an incoming handshake is proof
        // that a real peer wants THIS torrent right now, so wake it instead of
        // refusing the connection. The node's rotation task pauses it again on
        // a later pass once it has no connected peers, so woken torrents can't
        // accumulate. Upstream: a handshake for a paused torrent is refused.
        // Set SI_WAKE_ON_DEMAND=0 to restore upstream behaviour.
        fn si_wake_on_demand() -> bool {
            use std::sync::OnceLock;
            static ON: OnceLock<bool> = OnceLock::new();
            // Env is read once; the quiet-hours gate is checked every time.
            *ON.get_or_init(|| {
                std::env::var("SI_WAKE_ON_DEMAND")
                    .map(|v| v != "0")
                    .unwrap_or(true)
            }) && crate::SI_ACCEPT_INCOMING.load(Ordering::Relaxed)
        }
        if torrent.is_paused() && si_wake_on_demand() {
            if let Err(e) = self.unpause(&torrent).await {
                debug!("wake-on-demand: could not unpause torrent {id}: {e:#}");
            } else {
                debug!("wake-on-demand: woke paused torrent {id} for {addr}");
            }
        }
