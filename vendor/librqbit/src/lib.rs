...
/// PATCHED FOR SERMONINDEX — global gate for serving incoming connections.
///
/// The SermonIndex node flips this to `false` during scheduled quiet hours
/// (e.g. Sunday services, when the building's bandwidth belongs to the
/// livestream) and back to `true` afterwards. While `false`, the
/// wake-on-demand path in `check_incoming_connection` refuses to wake paused
/// torrents; combined with the node pausing everything live, the seed goes
/// fully silent without shutting down. Upstream librqbit has no such gate.
pub static SI_ACCEPT_INCOMING: std::sync::atomic::AtomicBool =
